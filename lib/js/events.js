...
let allHeroContainers = document.getElementsByClassName( "hero_container" );
...
function removeIds()
{
    //get all again in case new heroes were added
    allHeroContainers = document.getElementsByClassName( "hero_container" );
    
    for ( let c of allHeroContainers )
    {
        c.removeAttribute( "id" );
    }
}

//** a little weird w/o context */
...
function highlightHero( e )
{
    removeIds();

    e.currentTarget.id = "highlight";
...
for ( let c of allHeroContainers )
{
    c.addEventListener( "click", highlightHero );
}
...
//** combined selectors for same element do NOT have spaces */
...
//done, looks weird right now b/c picture stretches
...
//** had to inspect CSS styling on text container to see original display value */
...
const type_in = document.getElementById( "type_input" );
const difficulty_in = document.getElementById( "difficulty_input" );
const saying_in = document.getElementById( "saying_input" );

function submitHandler()
{
    event.preventDefault(); //do not reload

    //if any value not given
    if ( !name_in.value || !type_in.value || !difficulty_in.value || !saying_in.value )
    {
        alert( "Please fill in all fields" );
        return;
    }

    //create hero object and fill in fields
    const newHero = {};
    newHero.name = name_in.value;
    newHero.type = type_in.value;
    newHero.difficulty = difficulty_in.value;
    newHero.saying = saying_in.value;
    //not given; default values
    newHero.img = "lucio.png"; 
    newHero.story = "..this character has no backstory...yet!";
    newHero.abilities = [ { "name": "default", "description": "unspecified default" } ];
    ...
    heroes.push( newHero );
    // and grid div
    const newDiv = heroCreator.createCharSheet( newHero );
    gridDiv.appendChild( newDiv );

    //add highlighting functionality to new div
    newDiv.addEventListener( "click", highlightHero );
}

const ourButton = document.getElementById( "submit_button" );
ourButton.addEventListener( "click", submitHandler );
