 ...
 */
heroCreator.createCharSheet = function( heroObj ){
    //make div
    const newDiv = document.createElement( "div" );
    newDiv.classList.add( "hero_container" );

    //get image
    const img = document.createElement( "img" );
    img.src = "assets/" + heroObj.img;
...
heroCreator.createInfoDiv = function( heroObj ){
    const newDiv = document.createElement( "div" );
    newDiv.classList.add( "hero_text_container" );

    const keys = Object.keys( heroObj ); //array of keys
    let p, value;
    for ( let attr of keys )
    {
        value = heroObj[attr];
        p = document.createElement( "p" );
        if ( Array.isArray( value ) ) //handle abilities array separately
        {
            p.innerText = attr + ": " + this.getAbilityString( value );
        }
        else if ( attr !== "img" )
        {
            p.innerText = attr + ": " + value;
        }
 ...
 * @return {String}
 */
heroCreator.getAbilityString = function( abilitiesArr ){
    let str = "";

    const last = abilitiesArr.length - 1;
    for ( let i = 0; i < last; i++ )
    {
        str += abilitiesArr[i].name + ", ";
    }

    str += abilitiesArr[last].name;
